...
//Folder with no parent folder -> root folder
...
exports.createFolder = [
  ...
  asyncHandler(async (req, res, next) => {
    ...
    await createFolder(newFolderName, req.user.id, parentFolder.id);
    return res.redirect(`/folder/${parentFolder.id}`); //Redirects to the same page
  }),
];

//Folder name edit form
...
exports.editFolder = [
  ...
  asyncHandler(async (req, res, next) => {
    ...
    return res.redirect(`/folder/${folder.parentId ?? ""}`); //Redirects to the page User selected folder from
  }),
];
...
//Deletes file from DB and Supabase
